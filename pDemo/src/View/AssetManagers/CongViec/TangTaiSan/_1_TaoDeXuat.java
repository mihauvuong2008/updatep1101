...
public class _1_TaoDeXuat extends Shell {
	private DE_XUAT dx;
	...
	private static Log log = LogFactory.getLog(_1_TaoDeXuat.class);
	...
	public static void main(String args[]) {
		try {
			...
			_1_TaoDeXuat shell = new _1_TaoDeXuat(display, user);
			shell.open();
			shell.layout();
	 ...
	 * @throws SQLException
	 */
	public _1_TaoDeXuat(Display display, NGUOIDUNG user) throws SQLException {
		super(display, SWT.SHELL_TRIM);
		setImage(SWTResourceManager.getImage(_1_TaoDeXuat.class, "/add-icon (1).png"));
		setLayout(new GridLayout(3, false));
		_1_TaoDeXuat.user = user;
		...
		label_3.setText("Đơn vị ban hành:");
		...
		DateTime dateTime_1 = new DateTime(composite, SWT.BORDER | SWT.LONG);
		...
		label_1.setText("Ngày nhận văn bản:");

		DateTime dateTime = new DateTime(composite, SWT.CALENDAR | SWT.LONG);
		dateTime.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false, 1, 1));

		Label label = new Label(composite, SWT.NONE);
		label.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false, 1, 1));
		label.setText("Thông tin đề xuất:");
		...
		btnNewButton.setImage(
				SWTResourceManager.getImage(_1_TaoDeXuat.class, "/javax/swing/plaf/basic/icons/JavaCup16.png"));
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					if (dx.getMA_TAPHOSO() <= 0) {
						...
						t.setTEN_TAPHOSO("Tập hồ sơ - Đề xuất Mua sắm, Tiếp nhận PTTS");
						t.setGIOITHIEU_TAPHOSO("Tập hồ sơ - Đề xuất Mua sắm, Tiếp nhận PTTS");
		...
		btnTip.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (checkTextNotNULL()) {
					...
					setVisible(false);
					_2_TaoCongviec_Tangtaisan tc = new _2_TaoCongviec_Tangtaisan(display, user, getShell(), dx);
					tc.open();
				} else {
	...
	protected void createContents() {
		setText("Nhận đề xuất Mua sắm, tiếp nhận PTTS");
		setSize(615, 380);
